...
Deno.serve(async (req: Request): Promise<Response> => {
  try {
    ...
    // Best-effort Slack reply — channel/messageTs may not be set if error was early
    try {
      const body2 = await req.clone().json().catch(() => null);
      const ch = body2?.event?.channel;
      const ts = body2?.event?.ts;
      if (ch && ts) {
        await replyInSlack(ch, ts, `⚠️ Failed to capture thought: ${err instanceof Error ? err.message : String(err)}`);
      }
